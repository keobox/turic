...
- Recap about video memory
  - function mutate, first part only the bitmap (1,0 is pixel is enabled)
  - There is a byte that has the attribute of every 8x8 bit group (pixel).
  - Minute 8:10 shows a table of the video memory
...
- Write with Claude Code a program to see the percentages
- Prompt: Take the file /tmp/zxfy.txt that was generated by the MacOS sapmple program and turn it into a
percentage of time used function. Turn this work into a python script that I can reuse.
Put the script somewhere into /tmp.
- Results 94.2% on computeDiff, is clear that this program is not optimized.
...
- Digress about dithering: computeDiff make the "image difference" using a pixel kernel of 2x2 pixels
  ...
  - Tried to remove the kernexl and the result is bad.
